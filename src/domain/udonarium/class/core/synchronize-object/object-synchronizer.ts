...
export class ObjectSynchronizer {
    ...
    EventSystem.register(this)
      ...
      .on(EVENT_NAME.REQUEST_GAME_OBJECT, 1000, (event) => {
        ...
        if (ObjectStore.instance.isDeleted(event.data)) {
          ...
          if (object)
            EventSystem.call(EVENT_NAME.UPDATE_GAME_OBJECT, object.toContext(), event.sendFrom);
        }
      })
  ...
  private createObject(context: ObjectContext) {
    const newObject: GameObject | null = ObjectFactory.instance.create(
      context.aliasName,
      context.identifier
    );
    if (!newObject) {
  ...
  private synchronize() {
    while (this.requestMap.size > 0 && this.tasks.length < 32) this.runSynchronizeTask();
  }
  ...
  private runSynchronizeTask() {
    ...
    task.onfinish = (_task) => {
      ...
      const _targetPeerIdTasks = this.peerMap.get(targetPeerId);
      if (_targetPeerIdTasks) _targetPeerIdTasks.splice(_targetPeerIdTasks.indexOf(_task), 1);
      this.synchronize();
    };
